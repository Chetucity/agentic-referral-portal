...
export default async function ReferralPage({
  ...
  // The resume attached to this referral, if there is one. Loaded here rather
  // than joined into getReferralDetail so the referral query stays unchanged
  // for the many callers that do not care about resumes.
  const attachedResume = referral.resumeId
    ? await db
        .select({
          id: resumes.id,
          title: resumes.title,
        ...
        .from(resumes)
        .where(eq(resumes.id, referral.resumeId))
        .get()
    : null;
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-[1fr_280px]">
        ...
        <aside className="space-y-4">
          <div className="card p-5">
            ...
            {/*
              A resume built in the portal is the preferred attachment: the
              referrer reads it here, signed in, rather than following a link
              to a file-sharing site they have to trust. An external link is
              still honoured for referrals that predate the builder.
            */}
            {attachedResume && (isSeeker || isReferrer || isCompanyRecruiter || isAdmin) && (
              <Link
                href={`/resumes/${attachedResume.id}`}
                className="btn-secondary btn-sm mt-4 flex w-full items-center justify-center gap-2"
              >
                Read resume
                {typeof attachedResume.atsScore === "number" && (
                  <span className="text-xs text-slate-500">
                    ATS {attachedResume.atsScore}/100
                  </span>
                )}
              </Link>
            )}
            ...
            {isSeeker && !attachedResume && (
              <Link href="/resume" className="btn-secondary btn-sm mt-4 w-full">
                Attach a resume
              </Link>
            )}
          </div>
