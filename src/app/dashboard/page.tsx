...
async function SeekerDash({ userId }: { userId: string }) {
  const refs = await listReferralsForSeeker(userId);
  const counts = countByStatus(refs);
  const me = await db.select().from(users).where(eq(users.id, userId)).get();
  const suggestions = await suggestedJobs(me?.skills ?? null, 3);
...
async function ReferrerDash({
  ...
  const refs = await listReferralsForReferrer(user.id);
  const counts = countByStatus(refs);
  const pending = refs.filter((r) => r.status === "REQUESTED");
  const companyJobs = user.company ? await listJobs({ companySlug: user.company.slug }) : [];
...
async function RecruiterDash({
  ...
  const jobs = await listJobs({ companySlug: user.company.slug }, true);
  const refs = await listReferralsForCompany(user.company.id);
  const counts = countByStatus(refs);
  const referrers = await listReferrers(user.company.id);
