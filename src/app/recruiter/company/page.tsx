...
export default async function CompanyProfilePage() {
  ...
  // The membership row carries a slug, but the company itself could have been
  // deleted since the session was issued — treat that like "not linked" rather
  // than crashing the page.
  const company = await getCompanyBySlug(user.company.slug);
  if (!company) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-8">
        <EmptyState
          title="That company no longer exists."
          action={{ href: "/profile", label: "Go to profile" }}
        />
      </div>
    );
  ...
  const team = await listCompanyTeam(company.id);
