...
export default async function Home() {
  ...
  const jobs = (await listJobs()).slice(0, 4);
  const companies = (await listCompanies()).slice(0, 6);

  const totalOpenings = (await listJobs()).reduce((n, j) => n + j.openings, 0);

  return (
    <>
      {/* Hero ------------------------------------------------------------- */}
      <section className="border-b border-brand-900/30 bg-gradient-to-br from-ink-950 via-ink-900 to-ink-800">
        <div className="mx-auto max-w-6xl px-4 pb-10 pt-16 sm:pb-12 sm:pt-24">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-4 text-4xl font-semibold tracking-tight text-white sm:text-5xl">
              ...
              <span className="text-brand-300">Then watch it move.</span>
            </h1>
