...
const requestSchema = z.object({
  ...
  /** A resume built in the portal. Empty string means "don't attach one". */
...
export async function requestReferral(
  ...
  const { jobId, referrerId, message, resumeId, resumeUrl } = parsed.data;
  ...
  // Only attach a resume the requester actually owns. The id arrives from a
  // form, so it is checked here rather than trusted.
  ...
  if (resumeId) {
    const owned = await db
      ...
      .get();
    if (!owned) return { error: "That resume isn't yours." };
    attachedResumeId = owned.id;
  }
...
export async function updateReferralStatus(
  ...
  const ref = await db.select().from(referrals).where(eq(referrals.id, referralId)).get();
  if (!ref) return { error: "Referral not found." };

  const job = (await db.select().from(jobs).where(eq(jobs.id, ref.jobId)).get())!;
