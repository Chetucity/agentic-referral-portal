import "server-only";
import { and, count, eq, like, or, sql, desc, inArray, ne } from "drizzle-orm";
import { db } from "@/db";
import {
  users,
  companies,
...
export async function listCompanies(q?: string): Promise<CompanyCard[]> {
  const rows = await db
    .select({
      id: companies.id,
      name: companies.name,
...
export async function getCompanyBySlug(slug: string) {
  return await db.select().from(companies).where(eq(companies.slug, slug)).get();
}

/** Members of a company who are open to referring, newest verified first. */
export async function listReferrers(companyId: string) {
  return await db
    .select({
...
export async function listCompanyTeam(companyId: string) {
  return await db
    .select({
      memberId: companyMembers.id,
      userId: users.id,
...
export async function listJobs(f: JobFilters = {}, includeClosed = false): Promise<JobCard[]> {
  const conds = [];
...
export async function jobLocations(): Promise<string[]> {
  const rows = await db
...
export async function listReferralsForSeeker(seekerId: string): Promise<ReferralRow[]> {
  return await db
    .select({
      ...referralSelect,
...
export async function listReferralsForReferrer(referrerId: string): Promise<ReferralRow[]> {
  return await db
    .select({
      ...referralSelect,
      counterpartId: users.id,
...
export async function listReferralsForCompany(companyId: string) {
  return await db
    .select({
      id: referrals.id,
      status: referrals.status,
...
export async function existingReferralsForJob(jobId: string, seekerId: string) {
  return await db
...
export async function platformStats() {
  /**
   * Counts go through the query builder rather than `sql.raw`, which the
   * libSQL driver types as an un-awaited statement. Drizzle's `count()` works
   * identically on both drivers and keeps the table names type-checked.
   */
  const scalar = async (q: Promise<{ c: number }[]>) => (await q)[0]?.c ?? 0;

  const [userCount, companyCount, jobCount, referralCount] = await Promise.all([
    scalar(db.select({ c: count() }).from(users)),
    scalar(db.select({ c: count() }).from(companies)),
    scalar(db.select({ c: count() }).from(jobs)),
    scalar(db.select({ c: count() }).from(referrals)),
  ]);

  const openings = await scalar(
    db
      .select({ c: sql<number>`coalesce(sum(${jobs.openings}), 0)` })
      .from(jobs)
      .where(eq(jobs.status, "OPEN")),
  );

  const hired = await scalar(
    db
      .select({ c: count() })
      .from(referrals)
...
export async function suggestedJobs(skills: string | null, limit = 4): Promise<JobCard[]> {
